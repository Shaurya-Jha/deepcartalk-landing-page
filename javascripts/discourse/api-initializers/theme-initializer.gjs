...
// decode HTML entities (turns "&rsquo;" into real character ’, etc.)
    ...
    Promise.all([ loadCustomEmojisOnce(), ajax("/hot.json") ])
      .then(([customMap, resp]) => {
        ...
        ul.innerHTML = topics.map(t => {
          ...
          // If not, fall back to decoding entities, escaping + replacing shortcodes via emoji map.
          let processed;
          if (unicode) {
            // unicode likely contains real emoji glyphs; still escape for safety
            processed = escapeHtml(String(unicode));
          } else {
            // decode server-sent entities first (fixes &rsquo; etc.), then escape and replace shortcodes
  ...
  function loadTop5() {
    ...
    ajax("/latest.json")
      .then((response) => {
        ...
        // prepare initial title safely (decode -> escape)
        const initialTitle = topicsWithImages[0] ? escapeHtml(decodeHtmlEntities(topicsWithImages[0].title || "")) : "";

        const carouselItemsHtml = topicsWithImages.map(t => {
          const decodedTitle = decodeHtmlEntities(t.title || "");
          const titleEscaped = escapeHtml(decodedTitle);
          const topicUrl = `/t/${t.slug || ""}/${t.id}`; // fallback: `/t/${t.id}`
          const imgSrc = normalizeUrl(t.image_url || t.image || NORMALIZED_PLACEHOLDER);

          return `
            <div class="carousel-item" data-title="${titleEscaped}">
              <a href="${topicUrl}" style="position:relative;display:block;">
                <img src="${imgSrc}" alt="${titleEscaped}" loading="lazy" />
            ...
            <div class="left-column">
              <div class="section-box carousel">
                ...
                <div style="position: absolute; bottom: 10px; left: 10px; color: white; background: rgba(0,0,0,0.5); padding: 5px; font-weight: bold;">
                  <span id="carousel-title">${initialTitle}</span>
                </div>
              </div>
  ...
  function initNewsCategories() {
    ...
    function renderCategoryGrid(topics, category, options = {}) {
      ...
      const itemsHtml = withImages.map(t => {
        // decode server entities then escape for safe insertion into attributes/markup
        const decodedTitle = decodeHtmlEntities(t.title || "");
        const titleEscaped = escapeHtml(decodedTitle);
        ...
        const alt = `Topic: ${decodedTitle || "Untitled"}`;

        return `
          <div class="news-item">
            <a href="${url}" class="thumb" aria-label="${titleEscaped}">
              <img loading="lazy" src="${mediaSrc}" alt="${titleEscaped}" onerror="this.onerror=null;this.src='${NORMALIZED_PLACEHOLDER}';">
              ${playOverlay}
            </a>
            <a href="${url}" class="title">${titleEscaped}</a>
            <div class="meta">${escapeHtml(poster ? poster + " · " : "")}${escapeHtml(postsCount ? postsCount + " posts · " : "")}${bumped ? escapeHtml(new Date(bumped).toLocaleDateString()) : ""}</div>
          </div>
        `;
      }).join("");
